 ...
 if ($DisplayAllCalendarsSharedTo -ne "")
{
  ...
  $global:ExportCSVFileName = "C:\O365-Audit\Results\$folderName\CalendarsSharedTo.$DisplayAllCalendarsSharedTo_$((Get-Date -format "MMM-dd hh-mm-ss tt").ToString()).csv" 
 }
 elseif ($ShowAllPermissions.IsPresent) 
 {
  $global:ExportCSVFileName = "C:\O365-Audit\Results\$folderName\AllCalendarPermissionReport-$((Get-Date -format "MMM-dd hh-mm-ss tt").ToString()).csv" 
 }
 elseif ($DefaultCalendarPermissions.IsPresent) 
 {
  $global:ExportCSVFileName = "C:\O365-Audit\Results\$folderName\DefaultCalendarPermissionsReport-$((Get-Date -format "MMM-dd hh-mm-ss tt").ToString()).csv" 
 }
 elseif ($ExternalUsersCalendarPermissions.IsPresent) 
 {
  $global:ExportCSVFileName = "C:\O365-Audit\Results\$folderName\SharedCalendarsWithExternalUsersReport-$((Get-Date -format "MMM-dd hh-mm-ss tt").ToString()).csv" 
 }
 else 
 {
  $global:ExportCSVFileName = "C:\O365-Audit\Results\$folderName\CalendarPermissionsReport-$((Get-Date -format "MMM-dd hh-mm-ss tt").ToString()).csv" 
 }
}
