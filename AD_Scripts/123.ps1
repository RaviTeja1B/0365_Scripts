...
if (-not (Get-Module -ListAvailable -Name ActiveDirectory)) {
    Write-Host "Active Directory module not found. Make sure RSAT is installed."
    exit
}
...
foreach ($group in $groups) {
    $groupData = Get-ADGroupMember -Identity $group.DistinguishedName | Select-Object Name, SamAccountName, objectClass
